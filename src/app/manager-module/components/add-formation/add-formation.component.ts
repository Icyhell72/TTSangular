...
export class AddFormationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.data!=null && this.data.id!=undefined){
      this._commun.getFormationById(this.data.id).subscribe({
        next: (result) => {
          this.f=result
  ...
  addFormation(elt: any,msg="Formation ajoutée avec succés") {
    ...
    this.addFormationSubscription = this.service.addFormation(formData).subscribe({
      next: (result) => {
  ...
  confirmFormation(elt:any) {
    ...
    this.service.confirmFormation(this.f.idFormation as number).subscribe({
      next: (result) => {
        this.spinner=true
      },
      error:(err)=>{
